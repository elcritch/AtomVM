...
proc debug_display_type*(t: term; ctx: ptr Context) =
...
proc debug_dump_binary_mem*(buf: cstring; val: term; n: cuint) =
  var i: cuint = 0
  while i < n:
...
proc debug_dump_term*(ctx: ptr Context; pos: ptr term; region: cstring; i: cuint) =
  var t: term = pos[]
...
proc debug_dump_context*(ctx: ptr Context) =
  debug_dump_heap(ctx)
...
proc reg_type_c*(reg_type: cint): char =
  case reg_type
  of 2:
    return 'a'
  of 3:
